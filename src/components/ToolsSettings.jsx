...
function ToolsSettings({ isOpen, onClose }) {
  ...
  const deleteMcpServer = async (serverId, scope = 'user') => {
    try {
      ...
      // Use agent CLI to remove the server
                ...
                <label className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-orange-900 dark:text-orange-100">
                      Full Auto Mode - Execute Tasks Automatically
                    </div>
                    <div className="text-sm text-orange-700 dark:text-orange-300">
                      Commands will be executed automatically (use with caution)
                    </div>
                  </div>
                </label>
              </div>
                  ...
                  {enableNotificationSound && (
                    <button
                      onClick={async () => {
                        ...
                        localStorage.setItem('qwen-tools-settings', JSON.stringify(currentSettings));
                      }}
