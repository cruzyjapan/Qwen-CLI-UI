...
// Setup file system watcher for agent projects folder using chokidar
async function setupProjectsWatcher() {
  const chokidar = (await import('chokidar')).default;
  const agentHome = process.env.AGENT_HOME || path.join(process.env.HOME, '.qwen');
  const qwenProjectsPath = process.env.AGENT_PROJECTS_DIR || path.join(agentHome, 'projects');
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      if (data.type === 'qwen-command') {
        ...
        if (isProcessing) {
          // Ignoring duplicate request - already processing
          return;
        }
        
        isProcessing = true;
        // Received qwen-command
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (data.type === 'init') {
        ...
        try {
          // Resolve interactive agent shell command - force using qwen
          let agentCmd = 'qwen';

          // If environment variable is set, use it (but warn if it's codex)
          if (process.env.AGENT_SHELL_CMD) {
            agentCmd = process.env.AGENT_SHELL_CMD;
            if (agentCmd.includes('codex')) {
              console.warn('[Shell Warning] AGENT_SHELL_CMD is set to codex, consider updating to qwen');
            }
          } else if (process.env.QWEN_PATH) {
            agentCmd = process.env.QWEN_PATH;
          }

          // Get the full path to qwen to avoid accidentally using codex
          let fullAgentPath = agentCmd;
          try {
            const whichResult = execSync(`which qwen`, { encoding: 'utf8' }).trim();
            if (whichResult) {
              fullAgentPath = whichResult;
              console.log('[Shell] Using Qwen CLI at:', fullAgentPath);
            }
          } catch (e) {
            console.log('[Shell] Could not find qwen, trying:', agentCmd);
          }

          // First check if the CLI is available
          try {
            execSync(`which ${fullAgentPath}`, { stdio: 'ignore' });
          } catch (error) {
            // console.error('❌ Agent CLI not found in PATH');
            ws.send(JSON.stringify({
              type: 'output',
              data: `\r\n\x1b[31mError: Agent CLI not found. Please check PATH or set AGENT_SHELL_CMD in .env\x1b[0m\r\n`
            }));
            return;
          }
          
          // Build shell command that changes to project directory first, then runs qwen
          let qwenCommand = fullAgentPath;

          if (hasSession && sessionId) {
            // Optional resume support via env flag
            const resumeFlag = process.env.AGENT_RESUME_FLAG; // e.g., --resume
            if (resumeFlag) {
              qwenCommand = `${fullAgentPath} ${resumeFlag} ${sessionId} || ${fullAgentPath}`;
            }
          }
  ...
  try {
    ...
    const storage = multer.diskStorage({
      destination: async (req, file, cb) => {
        const uploadDir = path.join(os.tmpdir(), 'qwen-ui-uploads', String(req.user.id));
